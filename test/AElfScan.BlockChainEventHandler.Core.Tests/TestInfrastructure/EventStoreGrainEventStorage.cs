...
public class EventStoreGrainEventStorage : IGrainEventStorage
{
    ...
    public EventStoreGrainEventStorage WithTypeResolver(Func<Type, string> typeToString,
    ...
    private async Task<(object Event, int EventNumber)> ReadLastEvent(string grainTypeName,
        GrainReference grainReference)
    {
        var streamName = GetStreamName(grainTypeName, grainReference);
        var slice = await _eventStoreConnection
    ...
    private EventStore.ClientAPI.EventData SerializeEvent(object @event)
    {
        ...
        return new EventStore.ClientAPI.EventData(Guid.NewGuid(), @event.GetType().Name, true, eventJson, metadataJson);
    }
    ...
    private (object Event, int EventNumber) DeserializeEvent(ResolvedEvent @event)
    {
        ...
        return (JsonConvert.DeserializeObject(eventJson, eventType, _jsonDefaultSettings),
            (int)@event.Event.EventNumber);
    }
}
