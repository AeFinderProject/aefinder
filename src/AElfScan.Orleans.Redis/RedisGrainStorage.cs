    ...
    public class RedisGrainStorage : IGrainStorage, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        public async Task ReadStateAsync(string grainType, GrainReference grainReference, IGrainState grainState)
        {
            ...
            catch (Exception e)
            {
                ...
                throw new RedisStorageException(Invariant($"ExceptionMsg:{e.Message}Failed to read grain state for {grainType} grain with id {grainReference} and storage key {key}."), e);
            }
        }
            ...
            catch (RedisServerException rse) when (rse.Message is not null && rse.Message.Contains(" WRONGTYPE ")) // ordinal comparison
            {
                ...
                if (!await MigrateAsync(key, newEtag, payload).ConfigureAwait(false))
                {
                    _logger.LogError(
                        ...
                        key,
                        rse.Message);
                    throw new RedisStorageException(Invariant($"Failed to write grain state for {grainType} grain with id {grainReference} with storage key {key} while migrating to new structure and error msg is {rse.Message}"));
                }
            }
            catch (Exception e)
            {
