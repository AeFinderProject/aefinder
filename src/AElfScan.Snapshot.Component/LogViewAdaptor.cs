...
using System.Linq;
using System.Threading.Tasks;using Force.DeepCloner;
using Microsoft.Extensions.Logging;
using Orleans.EventSourcing.Common;
using Orleans.LogConsistency;
using Orleans.Storage;
        ...
        where TLogEntry : class
    {
        ...
        protected override async Task ReadAsync()
        {
            ...
            while (true)
            {
                try
                {
                    Services.Log(LogLevel.Debug, "prepare read grainStorage.. {0}", "");
                    await _grainStorage.ReadStateAsync(_grainTypeName, Services.GrainReference, _snapshotState);
                    Services.Log(LogLevel.Debug, "already read .. {0}", "");
                    await EnsureStateGlobalVersion();
                    Services.Log(LogLevel.Debug, "ensure state.. {0}", "");
